...
def str_to_int(num_list: List[str]) -> List[int]:
    """Returns int list of num_list by changing each item in num_list into an
    int and serves as a helper function for clean_data
...
def get_votes_in_riding(data: List['VoteData'],
    ...
    ...             [117, 21, ['GREEN', 'LIBERAL', 'NDP', 'CPC'], [5, 5, 5, 4],
    ...              [True, True, True, True]]]
    >>> get_votes_in_riding(SAMPLE_DATA_2, 117) == exp_2
    True
    """
...
def voting_range(range_ballots: List[List[int]],
    ...
    ...              SAMPLE_ORDER_1)
    [7, 12, 6, 8]
    >>> voting_range([[1, 3, 4, 5, 0], [5, 5, 1, 2, 9], [1, 4, 1, 1, 4]],
    ...              SAMPLE_ORDER_2)
    [7, 12, 6, 8, 13]
    """
    voting_list = []
...
def get_lowest(party_tallies: List[int], party_order: List[str]) -> str:
    ...
    'LIBERAL'
    >>> get_lowest([1, 1, 1, 1, 1], SAMPLE_ORDER_2)
    'CPC'
    """
...
def get_winner(party_tallies: List[int], party_order: List[str]) -> str:
    ...
    'NDP'
    >>> get_winner([200, 100, 4, 200], SAMPLE_ORDER_1)
    'CPC'
    """
...
def party_order_remove(party_order: List[str], party: str)-> List[str]:
    ...
    party_order and serves as a helper function for voting_irv

    >>> party_order_remove(['GREEN', 'CPC'], 'CPC')
    ['GREEN']
    >>> party_order_remove(['GREEN','CPC', 'NDP'], 'GREEN')
    ['CPC', 'NDP']
    """
    for item in party_order:
        if item == party:
...
def equiv_items(item1: str, item2: str, value: int)-> int:
    ...
    function for voting_borda and voting_irv
